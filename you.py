...
if button_clicked:
        ...
        if collection1.find_one({'channel_information.Channel_Id': channel_id }):
                st.success ("channel id exists in MongoDB !!! please try other channel id")
        else:
              data_to_mongodb(channel_id)  
              st.success("Scrape and Update Data to MongoDB Successfully")
...
if button_sql:
    if  ip_channel in channel_ids:
        st.success("channel exists in SQL")
    else:
